...
const TechComponent: React.SFC<TechComponentProps> = ({ data, filters, handleCheck }) => {
  const techCards = data.map(
    d =>
      d.data.length > 0 && (
        <Grid style={{ marginTop: '12px', textAlign: 'center' }} item xs={12} md={5}>
  ...
  const filterBoxes = filters.map(filter => (
    <FilterBox label={filter.charAt(0).toUpperCase() + filter.slice(1)} value={filter} handleFilter={handleCheck} />
  ));
